...
const FoodList = () => {
  ...
  const getFoods = async () => {
    try {
      const response = await axiosCoach.get(`/getfoods/${state.id}`);
  ...
  return (
    ...
    <div className=" flex justify-center items-center flex-col">
     ...
     <Link to='/clientdiet' className=" no-underline text-lg flex items-center space-x-1 bg-white bg-opacity-40 px-3 py-1 rounded-2xl font-[Montserrat]"> <BiArrowToLeft size={'22px'} /> <h1>Go Back</h1> </Link>
      </div>
      ...
      {foods.map((food) => ( 
        <div className=" flex items-center space-x-7 justify-start pl-6 text-white bg-[#28282B] backdrop-blur-lg w-full py-4 rounded-3xl gap-1 shadow-xl hover:shadow-2xl hover:scale-105 duration-300 px-3 overflow-hidden">
            <img
              ...
              />
          <div>
            <div className=" text-2xl font-bold uppercase">{food.name}</div>
            <div className=" text-lg"><span className=" font-bold text-principal">quantite :</span> {food.quantite} </div>
            <div className=""><span className=" font-bold text-principal">Description : </span>  {food.description} </div>
          </div>
        </div>
            ))} 
      </div>
      ...
      <h1 className=" text-white p-4  text-2xl w-full font-poppins font-bold uppercase"> currently you don't have a foods in your diet wait your coach! </h1>
      </div>
      }
    </div>
          ...
          >
            <div className="relative w-auto my-6 mx-auto max-w-3xl">
              ...
              <div className="overlay  mr-2 mt-2 text-black " onClick={() => setShowModal(false)}></div>
                <img className="rounded-3xl w-full h-full shadow-3xl " src={photo} alt="" />
